...
dummy_data <- 
  ...
  select(CR) |> 
  slice(1:2) |>
  mutate(authors_1 = str_remove(CR, 
                                "\\([0-9]{4}\\) .*"),
         authors_2 = str_extract(authors_1,
                                 ".*,"),
         authors_3 = str_replace_all(authors_2,
                                     "\\.", 
                                     replacement = "")) |> 
  separate_rows(authors_3, sep = ",") |> 
  nest(authors = authors_3)

author_ref_net <- 
  data_tidied_scopus_ref |>
  select(CR) |> 
  slice(3) |>
  mutate(authors = str_remove(CR, 
                              "\\([0-9]{4}\\) .*"),
...
dummy_1 <- unite(author_ref_net, string, X1:X2, sep = " ") 
dummy_2 <- unite(dummy_1, string_2, X3:X4, sep = " ") # enough
dummy_3 <- unite(dummy_2, string_3, c("string", "string_2"), sep = "; ") 

for (i in dim(author_ref_net)[2]) {
  
  df <- unite(author_ref_net, string) # Select column 1 and 2
  
}

for (i in length(author_ref_net$authors)) {
  
  author_1 <- 
    str_c(author_ref_net |> slice(i) |> pull(), " ",
          author_ref_net |> slice(i + i) |> pull()) |> 
    tibble() |> 
    rename(author = 1)
...
x <- author_ref_net |> pull()
m <- rollapply(data = x, 2, by=2, c)    # Creates a Matrix of rows = n-1, each row as a List
l <- split(m, row(m)) 
l
dummy_data_1 <- 
  dummy_data[[4]][[1]]
...
dummy_data_2 |> 
  filter(authors_3 != "") |> 
  mutate(authors_3 = str_trim(authors_3)) |> 
  t() |> 
  tibble() |> View()
```

Maybe with bibliometrix but not.... 
