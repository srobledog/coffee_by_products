...
g_journal_husk <-
  ...
  scale_y_continuous(breaks = seq(min(df2_journal_husk_plot$year), 
                                  max(df2_journal_husk_plot$year), 
                                  by = 1)) + 
  ...
  labs(title = "Husk Top-Journals' Production over the Time", 
       x = "Journal", y = "") + 
  geom_line(data = df2_journal_husk_plot, 
            aes(x = Journal, y = year, group = Journal),
            size = 1, color = "black", alpha = 0.3) + 
  # scale_x_discrete(limits = rev(df2_journal_husk_plot$Journal)) + 
  coord_flip()
```
...
g_journal_pulp <- 
  ...
  ggplot(aes(y = PY, 
             ...
             text = paste("Journal: ",
                          ...
                          round(TCpY,
                                2)))) + 
  geom_point(aes(alpha = M_journal_pulp_plot |> 
                   pull(TCpY),
                 size = freq), 
             color = "black") +
  scale_size(range = c(2, 6)) +
  scale_alpha(range= c(0.3, 1)) +
  scale_y_continuous(breaks = seq(min(M_journal_pulp_plot |> 
                                        pull(PY)),
                                  max(M_journal_pulp_plot |> 
                                        pull(PY)),
                                  by = 1)) +
  guides(size = guide_legend(order = 1,
                             "N. Articles"),
  ...
  labs(title = "pulp Top-Journals' Production over the Time", 
       x = "Journal", y = "") + 
  geom_line(data = M_journal_pulp_plot, 
            aes(x = SO, y = PY, group = SO),
            size = 1, color = "black", alpha = 0.3) + 
  # scale_x_discrete(limits = rev(M_journal_pulp_plot$SO)) + 
  coord_flip()
```
...
M_journal_silverskin <- 
  ...
  separate_rows(byproduct, sep = ", ") |> 
  dplyr::filter(byproduct %in% "silverskin") |> 
  dplyr::select(SO, PY, TC) |> 
  dplyr::mutate(TCpY = TC/(2021 - PY + 1)) |> 
  add_count(SO, PY) |> 
  dplyr::rename(freq = n)
```
...
g_journal_silverskin <- 
  ...
  ggplot(aes(y = PY, 
             ...
             text = paste("Journal: ",
                          ...
                          round(TCpY,
                                2)))) + 
  geom_point(aes(alpha = M_journal_silverskin_plot |> 
                   pull(TCpY),
                 size = freq), 
             color = "black") +
  scale_size(range = c(2, 6)) +
  scale_alpha(range= c(0.3, 1)) +
  scale_y_continuous(breaks = seq(min(M_journal_silverskin_plot |> 
                                        pull(PY)),
                                  max(M_journal_silverskin_plot |> 
                                        pull(PY)),
                                  by = 1)) +
  guides(size = guide_legend(order = 1,
                             "N. Articles"),
  ...
  labs(title = "Silverskin Top-Journals' Production over the Time", 
       x = "Journal", y = "") + 
  geom_line(data = M_journal_silverskin_plot, 
            aes(x = SO, y = PY, group = SO),
            size = 1, color = "black", alpha = 0.3) + 
...
```{r}
M_journal_spent_coffee_ground_plot <- 
  M_journal_spent_coffee_ground |> 
  filter(PY >= 2011) |> 
  arrange(desc(PY)) |> 
  slice(1:11)
...
g_journal_spent_coffee_ground <- 
  ...
  ggplot(aes(y = PY, 
             ...
             text = paste("Journal: ",
                          ...
                          round(TCpY,
                                2)))) + 
  geom_point(aes(alpha = M_journal_spent_coffee_ground_plot |> 
                   pull(TCpY),
                 size = freq), 
             color = "black") +
  scale_size(range = c(2, 6)) +
  scale_alpha(range= c(0.3, 1)) +
  scale_y_continuous(breaks = seq(min(M_journal_spent_coffee_ground_plot |> 
                                        pull(PY)),
                                  max(M_journal_spent_coffee_ground_plot |> 
                                        pull(PY)),
                                  by = 1)) +
  guides(size = guide_legend(order = 1,
                             "N. Articles"),
  ...
  theme(legend.position = "right", text = element_text(color = "black", 
        ...
        axis.text.y = element_text(face = "bold", 
         ...
         legend.text = element_text(size = 15),
        legend.title = element_text(size = 15)) + 
  labs(title = "Spent Coffee Grounds Top-Journals' Production over the Time", 
       x = "Journal", y = "Year") + 
  geom_line(data = M_journal_spent_coffee_ground_plot, 
            aes(x = SO, y = PY, group = SO),
            size = 1, color = "black", alpha = 0.3) + 
  # scale_x_discrete(limits = rev(levels(M_journal_spent_coffee_ground_plot$PY))) + 
  coord_flip()
```
...
husk_authors_top <- 
  ...
  dplyr::count(AU, sort = TRUE)

husk_authors <- 
  data_tidied |>
  separate_rows(byproduct, sep = ", ") |> 
  dplyr::filter(byproduct %in% "husk") |> 
  separate_rows(AU, sep = ";") |>
  dplyr::select(AU, PY, TC) |> 
  dplyr::group_by(AU, PY) |> 
  add_count(AU, PY) |> 
  dplyr::summarise(TC_total = sum(TC),
                   freq = unique(n),
                   .groups = "drop") |> 
  dplyr::mutate(TCpY = TC_total/(2021 - PY + 1)) 
```
...
husk_authors_plot <- 
  ...
  theme(legend.position = "right", text = element_text(color = "black", 
        ...
        axis.text.x = element_text(face = "bold", 
                                   angle = 90), 
        axis.text.y = element_text(face = "bold")) + 
  labs(title = "Husk Top-Authors' Production over the Time", 
       x = "Author", y = "") + 
...
pulp_authors_plot <- 
  ...
  theme(legend.position = "right", text = element_text(color = "black", 
        ...
        axis.text.x = element_text(face = "bold", 
                                   angle = 90), 
        axis.text.y = element_text(face = "bold")) + 
  labs(title = "Pulp Top-Authors' Production over the Time", 
       x = "Author", y = "") + 
...
spent_coffee_grounds_authors_plot <- 
  ...
  theme(legend.position = "right", 
        ...
        axis.text.x = element_text(face = "bold", 
                                   angle = 90), 
        axis.text.y = element_text(face = "bold")) + 
  labs(title = "Spent Coffee Ground Top-Authors' Production over the Time", 
       x = "Author", y = "Year") + 
...
silverskin_authors_plot <- 
  ...
  theme(legend.position = "right", text = element_text(color = "black", 
        ...
        legend.text = element_text(size = 15),
        legend.title = element_text(size = 15)) + 
  labs(title = "Silverskink Top-Authors' Production over the Time", 
       x = "Author", y = "") + 
...
author_scientific_production <- 
  ggarrange(
            ggarrange(husk_authors_plot, pulp_authors_plot, 
                      ncol = 2, labels = c("a", "b"), 
                      align = "h",widths = c(1.5,2)), 
            ggarrange(silverskin_authors_plot, spent_coffee_grounds_authors_plot,
                      ncol = 2, labels = c("c", "d"), 
                      align = "h",widths = c(1.5,2)), 
            nrow = 2, 
            heights = c(1, 1)
...
```{r}
data_tidied_scopus <- # Create a list with single names
  data_tidied |> 
...
journal_citation_graph_weighted <- 
  graph_from_adjacency_matrix(as.matrix(get.adjacency(graph.data.frame(journal_citation_graph))), weighted = TRUE, mode = "undirected", diag = TRUE) |> 
  simplify() |> 
  giant.component()


journal_citation_graph_1 <-  # The graph is very dense we need to remove links
  delete.vertices(journal_citation_graph_weighted,
                  which(degree(journal_citation_graph_weighted, 
                               mode = "all") == 1))



byproducts_journals_citation <- 
  journal_citation_graph_1 |> 
  add_clusters_to_nodes()
...
country_collab <- 
  biblioNetwork(data.frame(data_tidied_au_co), 
...
dummy_1 <- 
  data_tidied_au_co |> 
  select(SR, AU_CO) |> 
  separate_rows(SR, sep = ";")

edgelist_countries <- tibble(Source = as.character(),
                             Target = as.character())

for (i in 1:length(dummy_1$SR)) {
...
```{r}
bio_comp_apps <- 
  data_tidied |> # Filter by applications
  separate_rows(byproduct, sep = ", ") |> # separate byproducts
  separate_rows(categories, sep = ", ") |> # separate string categories
  filter(categories == "bioactive compounds") |> # Select bioactive compounds
  separate_rows(tags, sep = ", ") |> # separate tags 
  select(byproduct, tags, categories, year) |> # Select columns needed
  filter(year >= 2010) |> # last 10 years 
  group_by(byproduct, tags, categories)|> # group some variables
  dplyr::count(year) # count by year


jitter <- position_jitter(width = 0.1, # prevent overlap
                          height = 0.2)
bio_comp_apps_plot <- # making the graph
  bio_comp_apps %>% 
  ggplot(aes(y = tags, 
             x = year, 
             size = n, 
             color = byproduct)) + # x, y and size 
  geom_point(position = jitter) + # add points to graph
  # geom_jitter(position = position_jitter(width = 0.1, # prevent overlap
  #                                        height = 0.1)) +
  scale_x_continuous(breaks = seq(min(bio_comp_apps$year), # x axis scale
                                  max(bio_comp_apps$year))) +
  labs(title = "Bioactive compounds applications", # titles 
       x = "Years",
       y = "Tags") +
  theme(#legend.position = "rigth",
    text = element_text(color = "black", # main titles format
                        face = "bold",
                        family = "Times New Roman"),
    plot.title = element_text(size = 25),
    axis.title.y = element_text(size = 20),
...
microbio_trans_apps <- 
  ...
  separate_rows(byproduct, sep = ", ") |> # separate byproducts
  separate_rows(categories, sep = ", ") |> # separate string categories
  filter(categories == "microbial transformation") |> # Select bioactive compounds
  separate_rows(tags, sep = ", ") |> # separate tags 
  select(byproduct, tags, categories, year) |> # Select columns needed
  filter(year >= 2010) |> # last 10 years 
  group_by(byproduct, tags, categories)|> # group some variables
...
jitter <- position_jitter(width = 0.1, # prevent overlap
                          height = 0.2)
microbio_trans_apps_plot <- # making the graph
  microbio_trans_apps %>% 
  ggplot(aes(y = tags, 
             x = year, 
             size = n, 
  ...
  scale_x_continuous(breaks = seq(min(bio_comp_apps$year), # x axis scale
                                  max(bio_comp_apps$year))) +
  labs(title = "Microbioal Transformation", # titles 
       x = "Years",
       y = "Tags") +
  theme(#legend.position = "rigth",
    text = element_text(color = "black", # main titles format
...
environmental_apps <- 
  ...
  separate_rows(byproduct, sep = ", ") |> # separate byproducts
  separate_rows(categories, sep = ", ") |> # separate string categories
  filter(categories == "environmental applications") |> # Select envir apps
  separate_rows(tags, sep = ", ") |> # separate tags 
  select(byproduct, tags, categories, year) |> # Select columns needed
  filter(year >= 2010) |> # last 10 years 
  group_by(byproduct, tags, categories)|> # group some variables
...
jitter <- position_jitter(width = 0.1, # prevent overlap
                          height = 0.2)
environmental_apps_plot <- # making the graph
  environmental_apps %>% 
  ggplot(aes(y = tags, 
             x = year, 
             size = n, 
  ...
  scale_x_continuous(breaks = seq(min(bio_comp_apps$year), # x axis scale
                                  max(bio_comp_apps$year))) +
  labs(title = "Environmental Applications", # titles 
       x = "Years",
       y = "Tags") +
  theme(#legend.position = "rigth",
    text = element_text(color = "black", # main titles format
...
environmental_apps_plot
```


### Biofuels from thermochemical processes 
...
biofuels_ther_apps <- 
  ...
  separate_rows(byproduct, sep = ", ") |> # separate byproducts
  separate_rows(categories, sep = ", ") |> # separate string categories
  filter(categories == "biofuels from thermochemical processes") |> # Select bio
  separate_rows(tags, sep = ", ") |> # separate tags 
  select(byproduct, tags, categories, year) |> # Select columns needed
  filter(year >= 2010) |> # last 10 years 
  group_by(byproduct, tags, categories)|> # group some variables
...
jitter <- position_jitter(width = 0.1, # prevent overlap
                          height = 0.2)
biofuels_ther_apps_plot <- # making the graph
  biofuels_ther_apps %>% 
  ggplot(aes(y = tags, 
             x = year, 
             size = n, 
  ...
  scale_x_continuous(breaks = seq(min(bio_comp_apps$year), # x axis scale
                                  max(bio_comp_apps$year))) +
  labs(title = "Biofuels from thermochemical processes", # titles 
       x = "Years",
       y = "Tags") +
  theme(#legend.position = "rigth",
    text = element_text(color = "black", # main titles format
...
materials_apps <- 
  ...
  separate_rows(byproduct, sep = ", ") |> # separate byproducts
  separate_rows(categories, sep = ", ") |> # separate string categories
  filter(categories == "materials") |> # Select bio
  separate_rows(tags, sep = ", ") |> # separate tags 
  select(byproduct, tags, categories, year) |> # Select columns needed
  filter(year >= 2010) |> # last 10 years 
  group_by(byproduct, tags, categories)|> # group some variables
...
jitter <- position_jitter(width = 0.1, # prevent overlap
                          height = 0.2)
materials_apps_plot <- # making the graph
  materials_apps %>% 
  ggplot(aes(y = tags, 
             x = year, 
             size = n, 
...
#   dplyr::filter(str_detect(byproduct, pattern = "husk")) %>%
#   dplyr::group_by(year,categories) %>%
...
#   ggplot(aes(y = categories, x = year, size = n)) +
#   geom_point(alpha = 1) +
...
data_tidied_pulp  <- 
  ...
  separate_rows(byproduct, sep = ", ") |> 
  separate_rows(categories, sep = ", ") |> 
  dplyr::filter(str_detect(byproduct, pattern = "pulp")) %>% 
  dplyr::group_by(year,categories) %>% 
  dplyr::count(categories, sort = TRUE) |> 
  dplyr::filter(year >= 2010)
...
data_tidied_silverskin  <- 
  ...
  separate_rows(byproduct, sep = ", ") |> 
  separate_rows(categories, sep = ", ") |> 
  dplyr::filter(str_detect(byproduct, pattern = "silverskin")) %>% 
  dplyr::group_by(year,categories) %>% 
  dplyr::count(categories, sort = TRUE) |> 
  dplyr::filter(year >= 2010)
...
data_tidied_spent_coffee_grounds  <- 
  ...
  separate_rows(byproduct, sep = ", ") |> 
  separate_rows(categories, sep = ", ") |> 
  dplyr::filter(str_detect(byproduct, pattern = "spent_coffee_grounds")) %>% 
  dplyr::group_by(year,categories) %>% 
  dplyr::count(categories, sort = TRUE) |> 
  dplyr::filter(year >= 2010)
