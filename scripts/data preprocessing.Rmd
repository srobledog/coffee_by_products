...
```{r}
spent_mendeley <- 
  revtools::read_bibliography("data/spent_coffee_grounds_79.bib") %>% 
  select(label, 
         doi, 
         mendeley_tags,
         title, 
...
silverskin_mendelely <- 
  revtools::read_bibliography("data/coffee_silverskin_35.bib") %>% 
  select(label, 
         doi, 
         mendeley_tags,
         title, 
...
```{r}
pulp_mendeley <- 
  revtools::read_bibliography("data/coffee_pulp_36.bib") %>% 
  select(label, 
         doi, 
         mendeley_tags,
         title, 
...
```{r}
husk_mendeley <- 
  revtools::read_bibliography("data/coffee_husk_25.bib") %>% 
  select(label, 
         doi, 
         mendeley_tags,
         title, 
...
```{r}
husk_scopus <- 
  bibliometrix::convert2df("data/raw data/Coffee Husk Scopus 21.bib", 
                           dbsource = "scopus", 
                           format = "bibtex")

pulp_scopus <- 
  bibliometrix::convert2df("data/raw data/Coffee Pulp Scopus 33.bib", 
                           dbsource = "scopus", 
                           format = "bibtex")

silverskin_scopus <- 
  bibliometrix::convert2df("data/raw data/Coffee Silverskin Scopus 34.bib", 
                           dbsource = "scopus", 
                           format = "bibtex")

spent_scopus <- 
  bibliometrix::convert2df("data/raw data/Spent coffee grounds Scopus 47.bib", 
                           dbsource = "scopus", 
                           format = "bibtex")
...
```{r}
husk_wos <- 
  bibliometrix::convert2df("data/raw data wos/Coffe Husk WOS 29.txt", 
                           dbsource = "wos", 
                           format = "plaintext")

pulp_wos <- 
  bibliometrix::convert2df("data/raw data wos/Coffee Pulp WOS 26.txt", 
                           dbsource = "wos", 
                           format = "plaintext")

silverskin_wos <- 
  bibliometrix::convert2df("data/raw data wos/Coffee Silverskin WOS 26.txt", 
                           dbsource = "wos", 
                           format = "plaintext")

spent_wos <- 
  bibliometrix::convert2df("data/raw data wos/Spent coffee grounds WOS 57.txt", 
                           dbsource = "wos", 
                           format = "plaintext")
...
I will remove "N/A"s tags

```{r}
mendeley_byproducts_no_nas <- 
  mendeley_byproducts %>% 
  filter(!str_detect(string = mendeley_tags, 
                     pattern = "n/a"))
dim(mendeley_byproducts_no_nas)
```

Other option to write the paper

```{r}
mendeley_byproducts_no_nas_other <- 
  mendeley_byproducts %>% 
  filter(!str_detect(string = mendeley_tags, 
                     pattern = "n/a"),
...
dim(mendeley_byproducts_no_nas_other)
```


Now, we need to remove papers with the tag "Review" and "na"
...
Analysing the number of NAs in DOIS
...
Let's manually assing identifiers to na dois
                       ...
                       ifelse(mendeley_tags_cate$label == dois_nas$label[4], 4,
                              ifelse(mendeley_tags_cate$label == dois_nas$label[5], 5,
                                     ifelse(mendeley_tags_cate$label == dois_nas$label[6], 6,
                                            ifelse(mendeley_tags_cate$label == dois_nas$label[7], 7,
                                                   ifelse(mendeley_tags_cate$label == dois_nas$label[8], 8, mendeley_tags_cate$doi))))))))
```
...
byproducts_raw_scopus_modified <- 
  read_csv("output/byproducts_raw_scopus_modified.csv")
```
